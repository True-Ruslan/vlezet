...
function measuredMetric(result, name, sourceLabel) {
  ...
  if (!metric || metric.status !== "measured" || typeof metric.value !== "number" || !Number.isFinite(metric.value)) {
    throw new Error(`${sourceLabel}: aggregate.${name} must be a finite measured metric.`);
  }
  return metric.value;
}
...
export function verifyM78COpeningGate(results) {
  ...
  for (const entry of results) {
    ...
    if (openingF1 < M7_8C_MINIMUM_OPENING_F1) {
      throw new Error(`${label}: Opening F1 ${openingF1.toFixed(6)} is below ${M7_8C_MINIMUM_OPENING_F1.toFixed(2)}.`);
    }
    ...
    if (incorrectHighConfidence !== 0) {
      throw new Error(`${label}: incorrect high-confidence rate must be 0, received ${incorrectHighConfidence}.`);
    }
  }
  return diagnostics;
}

async function readResult(path, label) {
  return { label, result: JSON.parse(await readFile(path, "utf8")) };
}

async function main(argv) {
  const paths = argv.slice(2);
  if (paths.length === 0 || paths.length > 2) {
    throw new Error("Usage: node m7-8c-opening-gate.mjs <core-result.json> [source-result.json]");
  }
  const labels = paths.length === 1 ? ["Core"] : ["Core", "Source"];
  const diagnostics = await Promise.all(paths.map((path, index) => readResult(path, labels[index])));
  const verified = verifyM78COpeningGate(diagnostics);
  for (const result of verified) {
    console.log(`${result.label}: Opening F1=${result.openingF1.toFixed(6)}, unknown-host=${result.unknownHosts}, stale=${result.staleDecisions}, incorrect-high-confidence=${result.incorrectHighConfidence}`);
  }
}
