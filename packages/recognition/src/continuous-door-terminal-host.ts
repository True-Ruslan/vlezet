...
function terminalOpeningForSegment(
  wall: WallGeometry,
  ...
  const endpointTolerancePx = Math.max(6, Math.min(16, wall.halfThicknessPx + 4));
  const startDistancePx = Math.abs(anchor.along);
  const endDistancePx = Math.abs(anchor.along - wall.lengthPx);
  if (Math.min(startDistancePx, endDistancePx) > endpointTolerancePx) return null;
  const terminalAlongPx = startDistancePx <= endDistancePx ? 0 : wall.lengthPx;
  const direction: -1 | 1 = terminalAlongPx === 0 ? -1 : 1;

  let consecutiveStructural = 0;
  let firstStructuralAlongPx: number | null = null;
  for (let distancePx = SCAN_STEP_PX; distancePx <= MAX_DOOR_WIDTH_PX; distancePx += SCAN_STEP_PX) {
    const alongPx = terminalAlongPx + direction * distancePx;
    if (!pointInsideRaster(pointOnWall(wall, alongPx), input.widthPx, input.heightPx)) break;
    const structural = crossSectionSupport(wall, alongPx, input.mask) >= STRUCTURAL_CROSS_SECTION_RATIO;
    if (structural) {
      consecutiveStructural += 1;
      if (firstStructuralAlongPx === null) firstStructuralAlongPx = alongPx;
      if (consecutiveStructural >= REQUIRED_STRUCTURAL_RUN_SAMPLES) break;
    } else {
      consecutiveStructural = 0;
      firstStructuralAlongPx = null;
    }
  }
  if (consecutiveStructural < REQUIRED_STRUCTURAL_RUN_SAMPLES || firstStructuralAlongPx === null) return null;

  const gapStartAlongPx = Math.min(terminalAlongPx, firstStructuralAlongPx);
  const gapEndAlongPx = Math.max(terminalAlongPx, firstStructuralAlongPx);
  const gapWidthPx = gapEndAlongPx - gapStartAlongPx;
  ...
  const anchorSideStart = terminalAlongPx - direction * boundaryProbePx;
  const anchorSideEnd = terminalAlongPx - direction * 3;
  const farSideStart = firstStructuralAlongPx + direction * 3;
  const farSideEnd = firstStructuralAlongPx + direction * boundaryProbePx;
  if (
    Math.min(anchorSideStart, anchorSideEnd) < -EPSILON
    || Math.max(anchorSideStart, anchorSideEnd) > wall.lengthPx + EPSILON
    || !pointInsideRaster(pointOnWall(wall, farSideStart), input.widthPx, input.heightPx)
    || !pointInsideRaster(pointOnWall(wall, farSideEnd), input.widthPx, input.heightPx)
  ) return null;
  if (averageSupport(wall, anchorSideStart, anchorSideEnd, input.mask) < MIN_BOUNDARY_SUPPORT_RATIO) return null;
  if (averageSupport(wall, farSideStart, farSideEnd, input.mask) < MIN_BOUNDARY_SUPPORT_RATIO) return null;

  const centerAlongPx = (gapStartAlongPx + gapEndAlongPx) / 2;
  const center = pointOnWall(wall, centerAlongPx);
  return {
    id: `terminal-door-${wall.candidate.id}-${Math.round(gapStartAlongPx)}-${Math.round(gapEndAlongPx)}`,
    kind: "door",
...
function equivalentOpening(
  ...
  return first.hostWallCandidateId === second.hostWallCandidateId
    && Math.hypot(
      (first.center.x - second.center.x) * widthPx,
      (first.center.y - second.center.y) * heightPx,
    ) <= 12
    && Math.abs((first.widthPx ?? 0) - (second.widthPx ?? 0)) <= 16;
}

export function detectTerminalHostDoorOpenings(
  input: ContinuousDoorHostAnalysisInput,
): ContinuousDoorHostAnalysisResult {
  if (
    ...
    return { openingHypotheses: [], diagnostics: ["terminal-host-door-invalid-mask"] };
  }
  if (
    input.wallCandidates.length > MAX_WALL_CANDIDATES
    || input.symbolSegments.length > MAX_SYMBOL_SEGMENTS
  ) {
    return { openingHypotheses: [], diagnostics: ["terminal-host-door-budget-exceeded"] };
  }
  ...
  for (const wall of walls) {
    for (const segment of segments) {
      const candidate = terminalOpeningForSegment(wall, segment, input);
      if (!candidate) continue;
      const duplicateIndex = openings.findIndex((existing) =>
        equivalentOpening(existing, candidate, input.widthPx, input.heightPx));
      if (duplicateIndex < 0) openings.push(candidate);
  ...
  const diagnostics = openingHypotheses.length > 0
    ? ["terminal-host-door-detected"]
    : [];
  if (openingHypotheses.length >= MAX_OPENING_HYPOTHESES) {
    diagnostics.push("terminal-host-door-result-budget-reached");
  }
