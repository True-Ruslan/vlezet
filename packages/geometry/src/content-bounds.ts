...
const DEFAULT_VIEWPORT: ViewportTransform = Object.freeze({
  offsetX: 140,
  offsetY: 140,
...
export function deriveDocumentBounds(
  document: FitDocumentLike,
  options: DeriveDocumentBoundsOptions = {},
): WorldBounds | null {
  ...
  for (const opening of document.openings) {
    ...
    try {
      for (const point of doorSwingPolygon(document, opening)) bounds = extend(bounds, point);
    } catch {
      // Invalid topology is shown elsewhere; bounds remain best-effort and never block the editor.
    }
  }
...
export function fitViewportToBounds(
  ...
  const rawScale = Math.min(availableWidth / worldWidth, availableHeight / worldHeight);
  const pixelsPerMillimeter = Math.max(MIN_SCALE, Math.min(MAX_SCALE, rawScale));
  const centerX = (bounds.minX + bounds.maxX) / 2;
  const centerY = (bounds.minY + bounds.maxY) / 2;
  return {
    pixelsPerMillimeter,
    offsetX: width / 2 - centerX * pixelsPerMillimeter,
    offsetY: height / 2 - centerY * pixelsPerMillimeter,
  };
}
