...
source = replace_once(
    ...
    "Canvas feedback overlay",
)
canvas_path.write_text(source)

css_path = Path("apps/web/app/m7-furniture-fit.css")
css = css_path.read_text()
if "\n.placement-fit-label {\n" in css:
    raise SystemExit("Canvas feedback CSS already exists")
css += '''
...
.placement-fit-label[data-fit-status="fits"] {
  border-color: #a7f3d0;
  background: rgba(236, 253, 245, 0.96);
  color: #067647;
}

.placement-fit-label[data-fit-status="tight"] {
  border-color: #fed7aa;
  background: rgba(255, 250, 235, 0.96);
  color: #b54708;
}

.placement-fit-label[data-fit-status="blocked"] {
  border-color: #fecaca;
  background: rgba(254, 243, 242, 0.96);
  color: #b42318;
}

.object-canvas-legend {
  position: absolute;
  z-index: 5;
  left: 14px;
  bottom: 52px;
...
.object-canvas-legend-swatch.is-recommended {
  border-top-color: #d97706;
  border-top-style: dashed;
}

.object-canvas-legend-swatch.is-actual {
...
marker = "\n  m7_7_canvas_patch:\n"
if marker not in ci:
    raise SystemExit("Temporary CI patch job marker is missing")
ci_path.write_text(ci.split(marker, 1)[0].rstrip() + "\n")

for temporary in [
    Path(".github/workflows/m7-7-canvas-patch.yml"),
    Path("scripts/m7-7-apply-canvas-patch.py"),
]:
    if temporary.exists():
        temporary.unlink()
