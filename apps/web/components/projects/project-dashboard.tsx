...
function projectFacts(project: VlezetProjectRecord): string {
  ...
  return `${roomCount} комнат · ${project.document.walls.length} стен · ${project.document.placedObjects.length} предметов`;
}
...
export function ProjectDashboard({
  ...
  const beginRename = (project: VlezetProjectRecord) => {
    ...
    <main className="projects-page">
      <header className="projects-header">
        <div className="projects-brand">
          <div className="brand-mark project-brand-mark">V</div>
        ...
        <div className="projects-header-actions">
          <input
            ...
            type="file"
            accept=".json,.vlezet.json,application/json"
            aria-label="Импортировать проект Vlezet"
            onChange={(event) => {
      ...
      <section className="projects-content">
        <div className="projects-intro">
          <div>
            ...
            <h1>Планировки, к которым можно вернуться</h1>
            <p>Проекты сохраняются автоматически в этом браузере на этом устройстве. Для резервной копии используйте экспорт Vlezet.</p>
          </div>
        ...
        {projects.length === 0 ? (
          <div className="projects-empty">
            ...
            <p>Нарисуйте стены, добавьте комнаты, двери, мебель и проверьте реальные зазоры.</p>
            <button className="primary-action empty-action" type="button" onClick={() => void onCreate()}>Создать проект</button>
          </div>
        ) : (
          <div className="project-grid">
            {projects.map((project) => (
              <article className="project-card" key={project.id}>
                <button className="project-card-open" type="button" onClick={() => void onOpen(project)} aria-label={`Открыть проект ${project.name}`}>
                  <div className="project-preview" aria-hidden="true"><span /><span /><span /></div>
                </button>
                <div className="project-card-body">
                  {editingId === project.id ? (
                    ...
                    <button className="project-title-button" type="button" onDoubleClick={() => beginRename(project)} onClick={() => void onOpen(project)}>{project.name}</button>
                  )}
                ...
                <div className="project-card-actions" aria-label={`Действия с проектом ${project.name}`}>
                  <button type="button" onClick={() => beginRename(project)}>Переименовать</button>
                  <button type="button" onClick={() => void onDuplicate(project)}>Копия</button>
                  <button className="project-delete-button" type="button" onClick={() => onRequestDelete(project)}>Удалить</button>
                </div>
              </article>
            ))}
          </div>
