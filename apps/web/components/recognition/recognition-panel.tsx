  ...
  return <>
    ...
    <aside className="reference-panel recognition-panel" aria-label="Распознавание плана">
      ...
      {draft && props.state.kind !== "running-local" ? <>
        ...
        {emptyDraft ? <div className="recognition-empty-state"><strong>Локальный CV ничего не выделил.</strong><p>Это не считается успешным распознаванием. Повторите анализ обновлённым локальным движком или сразу подключите AI-проверку — исходный план и геометрия не изменятся.</p><div className="recognition-inline-actions"><button className="secondary-action" type="button" onClick={props.onStartLocal}>Повторить локально</button><button className="primary-action" type="button" onClick={props.onRunCloud}>Проверить с AI</button></div></div> : null}
