...
describe("editor store topology", () => {
  ...
  it("chains from the exact committed vertex identity", () => {
    ...
    expect(store.getState().draftWall).toEqual({
      ...
      snap: noSnap(2500, 700),
      startTarget: { kind: "vertex", vertexId: "vertex-2", point: { x: 2500, y: 700 } },
      endTarget: null,
    });
  });
  ...
  it("closes a contour by reusing an existing vertex instead of duplicating coordinates", () => {
    ...
    store.getState().updateDraftWall(noSnap(3000, 0));
    store.getState().commitDraftWall();
    store.getState().updateDraftWall(noSnap(3000, 3000));
    store.getState().commitDraftWall();
    ...
    expect(store.getState().history.document.vertices).toHaveLength(3);
    expect(store.getState().history.document.walls.at(-1)?.endVertexId).toBe("vertex-1");
  });
  ...
  it("creates and undoes a T-junction plus partition as one semantic history entry", () => {
    ...
    store.getState().beginWall({ x: 3000, y: 2500 });
    store.getState().updateDraftWall(noSnap(3000, 0), {
      kind: "wall",
      wallId: "wall-1",
      point: { x: 3000, y: 0 },
    ...
    expect(after.history.document.walls.find((wall) => wall.id === "wall-1")?.junctionVertexIds).toHaveLength(1);
    expect(after.history.document.walls).toHaveLength(2);

    store.getState().undo();
    expect(store.getState().history.document.walls).toHaveLength(1);
    expect(store.getState().history.document.walls[0]?.junctionVertexIds).toEqual([]);
    store.getState().redo();
    expect(store.getState().history.document.walls).toHaveLength(2);
  });
  ...
  it("changes exact wall length and physical thickness through semantic commands", () => {
    ...
    store.getState().setSelectedWallThickness(240);

    const state = store.getState();
    const end = state.history.document.vertices.find((vertex) => vertex.id === "vertex-2");
    expect(end?.position.x).toBeCloseTo(6000, 10);
    expect(end?.position.y).toBeCloseTo(8000, 10);
    expect(state.history.document.walls[0]?.thickness).toBe(240);
